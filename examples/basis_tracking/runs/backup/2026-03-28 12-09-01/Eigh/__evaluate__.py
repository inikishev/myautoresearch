    ...
    def run(self):
        ...
        for m, n in shapes:
            ...
            for i in range(200):
                ...
                # check orthogonality
                self.log_step(i, f"orthogonality ({m}x{n})", torch.linalg.norm(B @ B.T - I).item())

                # check commutativity
                BtAB = B.T @ A_ref @ B
                diag_norm = torch.linalg.vector_norm(BtAB.diagonal())
                full_norm = torch.linalg.vector_norm(BtAB).clip(min=1e-10)
                self.log_step(i, f"explained variance ({m}x{n})", (diag_norm / full_norm).item()) # score is 0 to 1.
                self.file_logger.info("%ix%i %i: sec=%.3f", m, n, i, sec)

            del updater, A_ref, I

        for k, maximize, weight, display_rank in [
            ("time per iter", False, 1, False),
            ("orthogonality", False, 1, False),
