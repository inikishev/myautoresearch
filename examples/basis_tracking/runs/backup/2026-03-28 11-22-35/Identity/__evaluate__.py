    ...
    def run(self):
        ...
        for m,n in shapes:
            ...
            for i in range(100):
                ...
                self.log_step(i, f"time per iter ({n}x{m})", sec)

                A_ref.lerp_(U @ U.T, 1-updater.beta)

                # check orthogonality
                self.log_step(i, f"orthogonality ({n}x{m})", torch.linalg.norm(B @ B.T - I).item())

                # check commutativity
                BtAB = B.T @ A_ref @ B
                diag_norm = torch.linalg.vector_norm(BtAB.diagonal())
                full_norm = torch.linalg.vector_norm(BtAB).clip(min=1e-10)
                self.log_step(i, f"explained variance ({n}x{m})", (diag_norm / full_norm).item()) # score is 0 to 1.

        for k, maximize, weight in [
            ("time per iter", False, 1),
            ("orthogonality", False, 1),
            ("explained variance", True, 4),
        ]:
            for m,n in shapes:
                value = self.logger.mean(f"{k} ({n}x{m})")
                mean_metrics[k].append(value)
                self.log_final(f"{k} ({n}x{m})", value, maximize=maximize, display_leaderboard=False, display_summary=False, weight=weight, is_main=False)

            # log mean metrics
            self.log_final(f"mean {k}", np.mean(mean_metrics[k]), maximize=maximize, is_main=False)

        t = self._metrics["mean time per iter"].value
        o = self._metrics["mean orthogonality"].value
